...
def extract_titles(content_dir: Path) -> dict[str, tuple[str, str]]:
    ...
    for dirname in reversed(["episodes", "articles"]):
        ...
        for md_file in dirpath.glob("*.md"):
            ...
            # Articles always win over episodes with same title
            ...
            if ":" in title:
                ...
                if len(short) > 3:
                    ...
                    if short not in titles or dirname == "articles":
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Interlink Lenny's Wiki pages")
    ...
    print(f"  Articles: {len(articles)}, Episodes: {len(episodes)}")
